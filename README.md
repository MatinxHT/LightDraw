...
- 通过两次点击绘制平面反光镜、光屏、凸透镜和凹透镜，移动鼠标时实时预览长度；
- 有限线段求交、多次镜面反射、光屏吸收和理想薄透镜近轴折射；
...
| 移动或调整元件 | 拖动元件主体可整体平移；拖动线光源、镜面、光屏或透镜的端点可固定另一端并拉伸、旋转，光路在拖动时实时刷新 |
| 删除元件 | 单击光源、镜面、光屏或透镜即可删除，成功删除后自动返回平移工具 |
| 使用任意绘制工具时按住右键拖动 | 临时平移画布 |
| 滚动鼠标滚轮 | 以当前指针位置为中心缩放 |
| 点光源 | 单击放置，向 360° 均匀发光，随后自动返回平移工具 |
| 线平行光源 | 两次点击确定发射线段，光线沿线段法线方向平行发射 |
| 平面反光镜 / 光屏 / 凸透镜 / 凹透镜 | 第一次点击确定起点，第二次点击确定长度和朝向，随后自动返回平移工具；光线命中光屏后终止且不会反射 |
| Esc | 取消正在放置的物件 |
...
场景使用 UTF-8 JSON，并通过 `dataVersion` 标记数据结构版本。当前版本为 `3`，并可继续读取版本 `1` 和 `2`：
